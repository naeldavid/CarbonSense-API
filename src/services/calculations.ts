import { CalculationResponse, BatchCalculationResponse } from '../types';
import { EmissionFactorsService } from './emissionFactors';
...
export class CalculationsService {
  ...
  static calculateTransportation(
    ...
    const factor = EmissionFactorsService.getFactorById(method);
  ...
  static calculateEnergy(
    ...
    const factor = EmissionFactorsService.getFactorById(energyType);
   ...
   */
  static calculateProductLifecycle(
    quantity: number,
    product: string
  ): CalculationResponse {
    const factor = EmissionFactorsService.getFactorById(product);
  ...
  static calculateBatch(
    calculations: Array<{
      type: 'transportation' | 'energy' | 'product';
      activityId: string;
      quantity: number;
    }>
   ...
   */
  private static getOffsetRecommendations(emissionsCO2eKg: number): string[] {
